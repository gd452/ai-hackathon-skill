...
## 만들 프로덕트 — 마크다운 TOC (목차) 생성기 CLI

markdown 파일을 받아 `##`, `###` 헤더로부터 목차(Table of Contents) 를 만들어주는 CLI 도구.

요구 사항:
- 입력: 파일 경로 또는 stdin
- 출력: anchor 링크가 포함된 markdown bullet list
- depth 옵션 (`--max-depth 3`)
- 기존 `<!-- TOC -->` ~ `<!-- /TOC -->` 블록 자동 교체 옵션 (`--insert`)
- 한글 헤더 anchor slugify (소문자 + 공백→하이픈 + 비ASCII 보존)

언어 자유 (Python / Bash / Node 등).
